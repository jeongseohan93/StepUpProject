...
public class PagingComment {
    public static String getPaging(String pageURL,int nowPage, int rowTotal,int blockList, int blockPage){
        ...
        sb.append("<div class='page-menu'>");
        
        //-----그룹페이지처리 이전 --------------------------------------------------------------------------------------------        
        if(isPrevPage){
            sb.append("<a href='javascript:comment_list(");
            sb.append(startPage-1);
            sb.append(")' class='arrow prev'></a>");
        }
        else
            sb.append("<span class='arrow prev disabled'></span>");
        
        //------페이지 목록 출력 -------------------------------------------------------------------------------------------------
        for(int i=startPage; i<= endPage ;i++){
            if(i>totalPage)break;
            if(i == nowPage){ //현재 있는 페이지
                sb.append("<a href='javascript:void(0)' class='now'>");
                sb.append(i);
                sb.append("</a>");
            }
            else{
        ...
        if(isNextPage){
            sb.append("<a href='javascript:comment_list(");
            sb.append(endPage + 1);
            sb.append(")' class='arrow next'></a>");
        }
        else
            sb.append("<span class='arrow next disabled'></span>");
